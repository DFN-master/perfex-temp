<script>
	...
	var wh_serial_number_as_mandatory = <?php echo get_option('wh_serial_number_as_mandatory'); ?>;
	(function($) {
		"use strict";  
	...
	$('.btn_submit_multiple_serial_number').on('click', function() {
		...
		if(valid_edit_multiple_transaction){
			...
			if(wh_serial_number_as_mandatory == 1 && serial_number_mandatory){
				alert_float('danger', "<?php echo _l('wh_serial_number_as_mandatory') ; ?>");
				return;
			}
	...
	$('.btn_auto_generate_serial_number').on('click', function() {
		...
		$.get(admin_url+'warehouse/generate_serial_number/' + serial_number_quantity).done(function(response){
