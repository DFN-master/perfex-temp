...
if (isset($arr_inventory_manage) && count($arr_inventory_manage) > 0) {
	...
	foreach ($arr_inventory_manage as $iv_key => $inventory_manages) {
		...
		if (isset($inventory_manages) && count($inventory_manages) > 0) {
			...
			$items .= '
			<th width="7%" align="left" ><strong>' . _l('wh_item_code') . '</strong></th>
			<th width="15%" align="left"><strong>' . _l('description') . '</strong></th>
			<th width="8%" align="left"><strong>' . _l('wh_item_type') . '</strong></th>
			<th width="7%" align="left"><strong>' . _l('wh_group') . '</strong></th>
			<th width="7%" align="left"><strong>' . _l('wh_sub_group') . '</strong></th>
			<th width="9%" align="left"><strong>' . _l('wh_batch_no') . '</strong></th>
			<th width="10%" align="left"><strong>' . _l('wh_serial_hashtag') . '</strong></th>
			<th width="7%" align="left"><strong>' . _l('expiry_date') . '</strong></th>
			<th width="6%" align="left"><strong>' . _l('wh_uom') . '</strong></th>
			<th width="6%" align="right"><strong>' . _l('wh_bal_qty') . '</strong></th>
			<th width="9%" align="right"><strong>' . _l('wh_unit_cost') . '</strong></th>
			<th width="9%" align="right"><strong>' . _l('wh_total_cost') . '</strong></th>
			</tr>
			</thead>
			...
			foreach ($inventory_manages as $commodity_id => $commodity_value) { 
				...
				foreach ($commodity_value as $key => $inventory_value) {
					...
					if (isset($serial_numbers[$inventory_value['id']]) && count($serial_numbers[$inventory_value['id']]) > 0) {
						foreach ($serial_numbers[$inventory_value['id']] as $serial_key => $serial_number) {
							...
							$items .= '<td align="right">' . app_format_number((float) $inventory_value['purchase_price'], '') . '</td>';
							$items .= '<td align="right">' . app_format_number($serial_number_total_cost, '') . '</td>';
							$items .= '</tr>';
						}
					}
					...
					if ((float) $inventory_value['inventory_number'] > (float) $total_serial_qty) {
						...
						$items .= '<td align="right">' . app_format_number((float) $inventory_value['purchase_price'], '') . '</td>';
						$items .= '<td align="right">' . app_format_number($no_serial_number_total_cost, '') . '</td>';
						$items .= '</tr>';
					}
				...
				$items .= '<th colspan="1" class="bold"></th>';
				$items .= '<th colspan="1" class="bold" align="right" style="border-top: 1px hair black !important; border-bottom: 1.5px hair black !important;"><span style="font-weight:bold;">' . app_format_number($total_cost, '') . '</span></th>';
				$items .= '</tr>';


				if(count($arr_inventory_manage) == $total_inventory_manage){
					$items .= '<tr style="' . $table_font_size . '">';
					$items .= '<th colspan="8" class="text-right bold" align="right"></th>';
					$items .= '<th colspan="1" class="bold"></th>';
					$items .= '<th colspan="1" class="bold" align="right"></th>';
					$items .= '<th colspan="1" class="bold"></th>';
					$items .= '<th colspan="1" class="bold" align="right"></th>';
					$items .= '</tr>';

					$items .= '<tr style="' . $table_font_size . '">';
					$items .= '<th colspan="8" class="text-right bold" align="right"><span style="font-weight:bold;">' . _l('wh_grand_total') . ' : </span></th>';
					$items .= '<th colspan="1" class="bold"></th>';
					$items .= '<th colspan="1" class="bold" align="right" style="border-top: 1px hair black !important; border-bottom: 1.5px hair black !important;"><span style="font-weight:bold;">' . app_format_number($grand_total_qty, '') . '</span></th>';
					$items .= '<th colspan="1" class="bold"></th>';
					$items .= '<th colspan="1" class="bold" align="right" style="border-top: 1px hair black !important; border-bottom: 1.5px hair black !important;"><span style="font-weight:bold;">' . app_format_number($grand_total_cost, '') . '</span></th>';
					$items .= '</tr>';
				}
			}
			$items .= '</tbody>
