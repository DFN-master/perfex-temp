...
foreach ($rResult as $aRow) {
    ...
    $_data .= '<a href="javascript:void(0)" onclick="edit_currency_rate_modal('.$aRow['id'].'); return false;" class="text-primary">' . _l('loy_edit') . '</a>';
    $_data .= ' | <a href="'.admin_url('loyalty/delete_currency_rate/'.$aRow['id'].'').'" class="text-danger _delete">' . _l('loy_delete') . '</a>';
    $_data .= '</div>'; 

    $row[] = $aRow['from_currency_name'].' '._l('loy_to').' '.$aRow['to_currency_name'].$_data;  
