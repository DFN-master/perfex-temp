...
foreach ($rResult as $aRow) {
   ...
   for ($i = 0; $i < count($aColumns); $i++) {
        ...
        if($aColumns[$i] == 'client'){
            ...
            if($aRow['type'] != 'remove' && $aRow['loyalty_point'] >= 0){
                $_data =  '<span class="label label-success">'._l('loy_'.$aRow['type']).'</span>';
            }else{
                if($aRow['type'] == 'remove'){
                    $_data =  '<span class="label label-danger">'._l('loy_'.$aRow['type']).'</span>';
                }else{
                    $_data =  '<span class="label label-danger">'._l('loy_remove').'</span>';
                }
            }
            ...
            if($aRow['type'] != 'remove' && $aRow['loyalty_point'] >= 0){
                ...
                if($aRow['type'] == 'remove'){
                    $_data = '<span class="label label-danger">-'.$aRow['loyalty_point'].'</span>';
                }else{
                    $_data = '<span class="label label-danger">'.$aRow['loyalty_point'].'</span>';
                }
            }
