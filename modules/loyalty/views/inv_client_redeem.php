...
<?php echo '<link href="' . module_dir_url(LOYALTY_MODULE_NAME, 'assets/css/mbs_program.css') . '?v=' . LOYALTY_REVISION . '"  rel="stylesheet" type="text/css" />'; ?>
<div class="col-md-6 col-md-offset-6">
<?php 
	$base_currency = get_base_currency_loy();

	$invoice_currency = loy_get_currency_by_id($invoice->currency);
	if($invoice_currency != ''){
...
<p class="text-info text-uppercase"><i class="fa fa-empire"></i><?php echo ' '._l('your_point').' '; ?><span class="label label-success"><?php echo client_loyalty_point($invoice->clientid); ?></span></p>
	...
	if($loy_rule){ ?>
		...
		if($loy_rule->redeemp_type == 'full'){
			...
			$min_redeem = $redemp_rule->point_weight*$min;
			if($min_redeem >=  $invoice->total){
				$min_redeem = ($invoice->total * $max_amount_received)/100;
			}
		}
	...
	<div class="col-md-5">
		...
		<div class="input-group" id="discount-total">
	     <input type="number" readonly class="form-control text-right" name="redeem_to" min="<?php echo loy_html_entity_decode($min_redeem); ?>" value="<?php echo loy_html_entity_decode($val_to); ?>" >
	     <div class="input-group-addon">
	        <div class="dropdown">
	           <span class="discount-type-selected">
	            <?php 
	            if($base_currency){
	            	echo loy_html_entity_decode($base_currency->symbol) ;
	        	}else{
	        		echo '';
	        	}
	            ?>
	...
	<button type="button" onclick="redeem_order(this); return false;" class="btn btn-icon btn-warning mtop25" data-symbol="<?php echo loy_html_entity_decode($base_currency->symbol); ?>" data-toggle="tooltip" data-placement="top" title="Redeem" ><i class="fa fa-refresh"></i></button>
