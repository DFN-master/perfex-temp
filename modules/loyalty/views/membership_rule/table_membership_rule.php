   ...
   for ($i = 0; $i < count($aColumns); $i++) {
        ...
        if($aColumns[$i] == 'name'){
            ...
            if ( (has_permission('loyalty', '', 'edit') || is_admin())) {
                $name = ' <a href="#" onclick="edit_mbs_rule('.$aRow['id'].',this); return false;" data-name="'.$aRow['name'].'" data-card="'.$aRow['card'].'" data-loyalty_point_from="'.$aRow['loyalty_point_from'].'" data-client="' . $aRow['client'] . '" data-client_group="' . $aRow['client_group'] . '" data-loyalty_point_to="'.$aRow['loyalty_point_to'].'" data-description="'.$aRow['description'].'" >'.$aRow['name'].'</a>';
            }

            $name .= '<div class="row-options">';

            if ( (has_permission('loyalty', '', 'edit') || is_admin())) {
                $name .= ' <a href="#" onclick="edit_mbs_rule('.$aRow['id'].',this); return false;" data-name="'.$aRow['name'].'" data-card="'.$aRow['card'].'" data-loyalty_point_from="'.$aRow['loyalty_point_from'].'" data-client="' . $aRow['client'] . '" data-client_group="' . $aRow['client_group'] . '"  data-loyalty_point_to="'.$aRow['loyalty_point_to'].'" data-description="'.$aRow['description'].'" >' . _l('edit') . '</a>';
            }
