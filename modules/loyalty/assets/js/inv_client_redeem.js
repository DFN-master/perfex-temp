  ...
  $('input[name="voucher"]').on('change', function(){ 
  	...
  	if(voucher != ''){
  		...
  		if(id != '' && id != null && id != undefined){
	        ...
	        data.clientid = id;
	        data.invoice_id = $('input[name="inv_id"]').val();
...
function auto_redeem(invoker, weight) {
	...
	if(val_to > max){
		...
		if(redeem_from > 1){
	 		...
	 		$('input[name="redeem_from"]').val(redeem_from);
	 		$('input[name="redeem_to"]').val(round_loy(max));
	 	}else{
	 		$('input[name="redeem_from"]').val(redeem_from);
	 		$('input[name="redeem_to"]').val(0);
	 	}
	}else{
...
function redeem_order(el){
	...
	max = (total*rate_percent)/100;
	if (window.confirm("Are you sure you want to make a point redemption?")) {
		if(val_to != ''){
			if(val_to <= max){
				if(new_discount != 0){
				...
				new_total = parseFloat(total) - new_discount + parseFloat(old_discount);
				...
				new_total = parseFloat(total) - parseFloat(new_discount) + parseFloat(old_discount);
			...
			$('.total').html(symbol+numberWithCommas(round_loy(new_total)));

			$('.redeem_inv_btn').removeAttr('disabled');
			$(el).attr('disabled', 'true');
			$('input[name="redeem_from"]').addClass('readonly');
		}else{
