...
//cronjob
hooks()->add_action('after_cron_run', 'cronjob_currency_rates');
//hooks()->add_action('loyalty_init',LOYALTY_MODULE_NAME.'_appint');
hooks()->add_action('pre_activate_module', LOYALTY_MODULE_NAME.'_preactivate');
hooks()->add_action('pre_deactivate_module', LOYALTY_MODULE_NAME.'_predeactivate');
...
function loyalty_add_footer_components() {
    ...
    if(!(strpos($viewuri, '/admin/loyalty/configruration?group=currency_rates') === false)){
        echo '<script src="' . module_dir_url(LOYALTY_MODULE_NAME, 'assets/js/currency_rate.js') . '?v=' . LOYALTY_REVISION . '"></script>';
    }
...
function init_loyalty_portal_menu()
{
    ...
    if(is_client_logged_in()){
        ...
        $subtract_client_ids = get_option('customers_ids_not_use_membership_tab');
        $subtract_client_group_ids = get_option('customers_group_ids_not_use_membership_tab');

        $client_arr = explode(',', $subtract_client_ids);
        $group_arr = explode(',', $subtract_client_group_ids);

        $CI = &get_instance();

        $CI->load->model('client_groups_model');

        $cli_groups = $CI->client_groups_model->get_customer_groups($client_id);

        $in_gr = 0;
        foreach($cli_groups as $gr){
            if(in_array($gr['groupid'], $group_arr)){
                $in_gr++;
            }
        }
...
function unset_invoice_redeem_data($data){
    ...
    if(isset($data['data']['min_point'])){
        $data['redeem']['min_point'] = $data['data']['min_point'];
        unset($data['data']['min_point']);
    }
 ...
 *
 */
function cronjob_currency_rates($manually) {
    $CI = &get_instance();
    $CI->load->model('loyalty/loyalty_model');
    if (date('G') == '18') {
        if(date('Y-m-d') != get_option('cur_date_cronjob_currency_rates')){
            $CI->loyalty_model->cronjob_currency_rates($manually);
        }
    }
}

function loyalty_appint(){
    // Verificação de licença removida
}

function loyalty_preactivate($module_name){
    // Processo de ativação removido
}

function loyalty_predeactivate($module_name){
    // Processo de desativação removido
}
