...
class Leadmanagerinit
{
    ...
    {
        $CI       = &get_instance();
        $verified = true; // Assume the module is verified by default

        // The entire block below is related to license key activation and verification.
        // It has been commented out to disable this functionality as per request.
        /*
        ...
        if (file_exists(__DIR__ . '/../config/token.php') && 4 == count($id_data)) {
            ...
            try {
                $data = LMJWT::decode($token, new LMKEY($id_data[3], 'HS512'));
                if (!empty($data)) {
            ...
            if ('' == $last_verification || (time() > ($last_verification + $seconds))) {
                ...
                try {
                    ...
                    $request  = Lm_Requests::post(LM_VAL_PROD_POINT, $headers, json_encode(['verification_id' => $verification_id, 'item_id' => $CI->config->item('product_item_id')]));
        ...
        if (!file_exists(__DIR__ . '/../config/token.php') && !$verified) {
            $last_verification = get_option($module_name . '_last_verification');
            if (((int)$last_verification + (168 * (3000 + 600))) > time()) {
                $verified = true;
            }
        }
        if (!$verified) {
