...
foreach ($rResult as $aRow) {
    ...
    $row[]    = $aRow['id'];
    $nameRow = '<a  href="' . admin_url('leads/index/' . $aRow['id']) . '" onclick="init_lead(' . $aRow['id'] . ');return false;" >' . $aRow['name'] . '</a>';
    ...
    if (has_permission('lead_manager', '', 'can_audio_call') && get_option('call_twilio_active')) {
        ...
        if (isset($callerIdNumber) && !empty($callerIdNumber)) {
            $allow_call = '<li><a href="javascript:void(0);" data-to="' . $aRow["phonenumber"] . '" data-from="' . $callerIdNumber . '" data-id="' . $aRow['id'] . '" data-is_lead="1" data-client_id = "" onclick="dialPhone(this);"><i class="fa fa-phone" aria-hidden="true" data-toggle="tooltip" data-title="Call"></i></a></li>';
        } else {
    ...
    if (has_permission('lead_manager', '', 'can_sms')) {
        $allow_sms = '<li><a href="javascript:void(0);" onclick="leadManagerMessage(' . $aRow['id'] . ');" data-toggle="tooltip" data-title="Message" ><i class="fa fa-comments" aria-hidden="true"></i></a></li>';
    }
