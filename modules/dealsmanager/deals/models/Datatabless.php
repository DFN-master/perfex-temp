 ...
 * @author NaYeM
 */
class Datatabless extends App_Model
{
    ...
    public function get_client_invoices($filterBy = null)
    {
        ...
        $all_invoice = make_deals_datatables($where, $where_in);
        return $all_invoice;
    }
    ...
    public function get_client_estimates($filterBy = null, $client_id = null)
    {
        ...
        $all_estimate = make_deals_datatables($where);;
        return $all_estimate;
    }
    ...
    public function get_payment($filterBy = null, $search_by = null)
    {
        ...
        $all_payments = make_deals_datatables($where);
    ...
    public function get_client_proposals($filterBy = null)
    {
        ...
        $all_proposal = make_deals_datatables($where);
        return $all_proposal;
    }
    ...
    public function get_assign_stocklist($id = NULL, $type = NULL)
    {
        ...
        $assign_stocklist = make_deals_datatables($where);

        if (!empty($assign_stocklist)) {
            foreach ($assign_stocklist as $stocklist) {
                $stock_info = get_deals_row('tbl_stock', array('stock_id' => $stocklist->stock_id));
                if (!empty($stock_info)) {
                    $stock_sub_info = get_deals_row('tbl_stock_sub_category', array('stock_sub_category_id' => $stock_info->stock_sub_category_id));
                    if (!empty($stock_sub_info)) {
                        $stock_category = get_deals_row('tbl_stock_category', array('stock_category_id' => $stock_sub_info->stock_category_id));
                    }
                }
    ...
    {
        $all_clock_history = make_deals_datatables();
        if (!empty($all_clock_history)) {
            foreach ($all_clock_history as $clock_history) {
                $clock_info = get_deals_row('tbl_clock', array('clock_id' => $clock_history->clock_id));
                $staff_info = get_deals_staff_details($clock_history->user_id);
