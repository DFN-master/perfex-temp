...
class Asaas_gateway extends App_gateway
{
    ...
    public function __construct()
    {
        ...
        $this->setSettings(array(
            ...
            array(
                'name' => 'url_apipinzap',
                'label' => "URL de API do PinZap <p>A url de API padrão do PinZap é: https://api.pinzap.pro/messages/send Não tenho um plano (<a href=\"https://pinzap.pro/\" target=\"_blank\">Quero Contratar</a>) R$49,99/mês </p>",
                'type' => 'input',
            ),
            array(
                'name' => 'api_keypinzap',
                'label' => "API Key PinZap (<a href=\"https://grmslab.com.br/gera_api\" target=\"_blank\">Criar API Key</a>) <p>Certifique-se de que sua chave de API esteja correta e que todas as permissões necessárias tenham sido concedidas. Para atualizar a chave, vá até 'Editar Conexão' e insira o token gerado por nossa plataforma no campo correspondente.</p>",
                'type' => 'input',
            ),
            array(
                'name' => 'contatopinzap',
                'label' => 'Whatsapp Recebedor das notificações',
                'type' => 'input',
            ),
            array(
                'name' => 'sandbox',
    ...
    public function charge_billet($invoice)
    {
        ...
        if (!$client->asaas_customer_id) {
            ...
            $email_client   = $this->ci->asaas_gateway->get_customer_customfields($client->userid, 'customers', 'customers_email_do_cliente');
            $address_number = $this->ci->asaas_gateway->get_customer_customfields($client->userid, 'customers', 'customers_numero_do_endere_o');
            ...
            $cliente_create = $this->create_customer($this->ci->base_api->getUrlBase(), $this->ci->base_api->getApiKey(), $post_data);
    ...
    public function get_webhook($api_key, $api_url)
    {
        ...
        "object" => $response["enabled"],
        "object" => $response["interrupted"],
        "object" => $response["apiVersion"],
        "object" => $response["authToken"],
        ];
        */
