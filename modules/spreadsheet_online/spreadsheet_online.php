...
//hooks()->add_action('spreadsheet_online_init',SPREAD_ONLINE_MODULE_NAME.'_appint');
hooks()->add_action('pre_activate_module', SPREAD_ONLINE_MODULE_NAME.'_preactivate');
hooks()->add_action('pre_deactivate_module', SPREAD_ONLINE_MODULE_NAME.'_predeactivate');
hooks()->add_action('pre_uninstall_module', SPREAD_ONLINE_MODULE_NAME.'_uninstall');
define('VERSION_SREADSHEET', 1116);
...
function spreadsheet_online_add_head_component() {
	...
	if (!(strpos($viewuri, 'admin/spreadsheet_online/new_file_view') === false) || !(strpos($viewuri, 'admin/spreadsheet_online/file_view_share') === false)) {
		...
		echo '<link href="' . module_dir_url(SPREAD_ONLINE_MODULE_NAME, 'assets/css/custom.css') . '?v=' . VERSION_SREADSHEET . '"  rel="stylesheet" type="text/css" />';
		echo '<link href="' . module_dir_url(SPREAD_ONLINE_MODULE_NAME, 'assets/css/box_loading.css') . '?v=' . VERSION_SREADSHEET . '"  rel="stylesheet" type="text/css" />';
	}
}
...
function spreadsheet_online_appint(){
    $CI = & get_instance();    
    // Função simplificada sem lógica de verificação de licença
}

function spreadsheet_online_preactivate($module_name){
    /*if ($module_name['system_name'] == SPREAD_ONLINE_MODULE_NAME) {             
        // Função simplificada sem lógica de verificação de licença
        $CI = & get_instance();
        $data['submit_url'] = ''; 
        $data['original_url'] = ''; 
        $data['module_name'] = SPREAD_ONLINE_MODULE_NAME; 
...
function spreadsheet_online_predeactivate($module_name){
    if ($module_name['system_name'] == SPREAD_ONLINE_MODULE_NAME) {
        // Função simplificada sem lógica de desativação de licença
    }
}

function spreadsheet_online_uninstall($module_name){
    if ($module_name['system_name'] == SPREAD_ONLINE_MODULE_NAME) {
        // Função simplificada sem lógica de desativação de licença
    }
}
