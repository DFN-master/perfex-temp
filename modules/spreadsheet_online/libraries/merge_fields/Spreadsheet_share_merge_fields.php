...
class spreadsheet_share_merge_fields extends App_merge_fields
{
    ...
    public function format($share_obj)
    {
        ...
        if($share_obj->type_template == "staff_template"){
            ...
            $url = admin_url('spreadsheet_online/file_view_share/'.$share_obj->parent_id.'/'.$share_obj->id);

            $fields['{share_link_spreadsheet}']   = $url;
            $fields['{receiver}']            = get_staff_full_name($share_obj->staff_share_id);        

        }elseif ($share_obj->type_template == "client_template") {
            $url = site_url('spreadsheet_online/spreadsheet_online_client/file_view_share/'.$share_obj->parent_id.'/'.$share_obj->id);

            $fields['{share_link_client_spreadsheet}'] = $url;
