    (function(){
      ...
      $('select[name="client_groups_share[0]"]').on('change', function(){
        var val = $(this).val();
        var name = $(this).attr("name");
        if(val != ''){
          requestGet(admin_url + 'spreadsheet_online/append_value_group/'+ val).done(function(response) {
            response = JSON.parse(response);
            $('select[name="clients_share[0]"]').html(response);
            $('select[name="clients_share[0]"]').selectpicker('refresh');
          })
        }else{
          requestGet(admin_url + 'spreadsheet_online/get_client_all').done(function(response_client) {
            ...
            $('select[name="clients_share[0]"]').html(response_client);
            $('select[name="clients_share[0]"]').selectpicker('refresh');
          })
        }
      })

      $('select[name="departments_share[0]"]').on('change', function(){
        var val = $(this).val();
        var name = $(this).attr("name");
        if(val != ''){
          requestGet(admin_url + 'spreadsheet_online/append_value_department/'+ val).done(function(response) {
            response = JSON.parse(response);
            $('select[name="staffs_share[0]"]').html(response);
            $('select[name="staffs_share[0]"]').selectpicker('refresh');
          })
        }else{
          requestGet(admin_url + 'spreadsheet_online/get_staff_all').done(function(response_staff) {
            ...
            $('select[name="staffs_share[0]"]').html(response_staff);
            $('select[name="staffs_share[0]"]').selectpicker('refresh');
          })
        }
      })
            ...
            if(response.group_share_client == 2){
              ...
              if(response.client_groups_share || response.clients_share){
                ...
                $.each(clients_share, function (index, value) {
                  ...
                  $('select[name="clients_share['+index+']"]').val(client_share_value).selectpicker('refresh');
            ...
            if(response.group_share_staff == 1){
              ...
              if(response.departments_share || response.staffs_share){
                ...
                $.each(staffs_share, function (index, value) {
                  ...
                  $('select[name="staffs_share['+index+']"]').val(staff_share_value).selectpicker('refresh');
    ...
    $('#ShareModal [type="submit"]').click(function() {
      ...
      if($('#ShareModal input[name="type"]:checked').val() == 'client'){
        if($('#ShareModal select[name="clients_share[]"]').val() == '' && $('#ShareModal select[name="client_groups_share[]"]').val() == ''){
          alert_float('warning', 'Please select at least one customer or a group of customers.!');
          $('#ShareModal [name="check"]').val('');
        }else{
          $('#ShareModal [name="check"]').val(1);
        }

      }else{
        if($('#ShareModal select[name="staffs_share[]"]').val() == '' && $('#ShareModal select[name="departments_share[]"]').val() == ''){
          alert_float('warning', 'Please select at least one staff or a department.!');
          $('#ShareModal [name="check"]').val('');
        }else{
          $('#ShareModal [name="check"]').val(1);
        }
    ...
    $('#spreadsheet-advanced .right-menu-position').on('click', function(){
    ...
    $(document).on("dblclick","#spreadsheet-advanced .right-menu-position",function() {
      var type = $(this).data("tt-type");
      var parent_id = $(this).data('tt-parent-id');
      parent_id = parent_id == undefined ? 0 : parent_id;
      var id_set = $(this).data('tt-id');
      var name = $(this).data('tt-name');
    ...
    $(document).ready(function() {
      ...
      $("body").on('click', '.new_box_information_review_client', function() {
        ...
        onchane_handle_client(addMoreBoxInformationReviewInputKey, client_share_value);
 ...
 $('select[name="departments_share['+index+']"]').on('change', function(){
  ...
  if(val != ''){
    requestGet(admin_url + 'spreadsheet_online/append_value_department/'+ val).done(function(response) {
      ...
      if(staff_share != ''){
        $('select[name="staffs_share['+index+']"]').val(staff_share).selectpicker('refresh');
      }
    })
  }else{
    requestGet(admin_url + 'spreadsheet_online/get_staff_all').done(function(response_staff) {
 ...
 $('select[name="client_groups_share['+index+']"]').on('change', function(){
  ...
  if(val != ''){
    requestGet(admin_url + 'spreadsheet_online/append_value_group/'+ val).done(function(response) {
      ...
      if(client_share != ''){
        $('select[name="clients_share['+index+']"]').val(client_share).selectpicker('refresh');
      }
    })
  }else{
    requestGet(admin_url + 'spreadsheet_online/get_client_all').done(function(response_client) {
...
function handle_click_checkbox(){
 ...
 $('input[type="checkbox"]').click(function(){
  ...
  if(hide == "1"){
    ...
    $('select[name*="role_staff"]').val(1).selectpicker('refresh');
    ...
    $('select[name*="role_client"]').val(1).selectpicker('refresh');
