...
/**
 * get_checkin_attachment
 * @param  array $array_checkin_ids
 * @return array
 */
function get_checkin_attachment($array_checkin_ids) {
	$list_ids = '(';
	for ($i = 0; $i < count($array_checkin_ids) - 1; $i++) {
		$list_ids .= $array_checkin_ids[$i] . ',';
	}
	$list_ids .= $array_checkin_ids[count($array_checkin_ids) - 1] . ')';
	$CI = &get_instance();
	$CI->db->where('rel_id IN ' . $list_ids);
	$CI->db->where('rel_type', 'checkin');
	return $CI->db->get(db_prefix() . 'files')->result_array();
}
...
/**
 * handle_checkin_attachments
 * @param  int $checkin_id
 * @param  string $index_name
 * @return object
 */
function handle_checkin_attachments($checkin_id, $index_name = 'attachments') {
	$path = OKR_MODULE_UPLOAD_FOLDER . '/checkin/' . $checkin_id . '/';
	$uploaded_files = [];
	if ($_FILES[$index_name]['name'][0] != "") {
		if (isset($_FILES[$index_name])) {
				...
				if ($i <= get_option('maximum_allowed_okrs_attachments')) {
					...
					if (!empty($tmpFilePath) && $tmpFilePath != '') {
						...
						if (move_uploaded_file($tmpFilePath, $newFilePath)) {
							...
							$CI->misc_model->add_attachment_to_database($checkin_id, 'checkin', $attachment);
						}
					}
				}
			}
...
function okr_html_entity_decode($str){
