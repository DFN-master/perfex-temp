...
define('SI_LEAD_FOLLOWUP_VALIDATION_URL','http://www.sejalinfotech.com/perfex_validation/validate.php');
...
hooks()->add_filter('other_merge_fields_available_for', 'si_lead_followup_hook_register_other_merge_fields');
hooks()->add_action('after_email_templates', 'si_lead_followup_hook_add_email_templates');
register_merge_fields('si_lead_followup/merge_fields/si_lead_followup_merge_fields');
...
function si_lead_followup_hook_settings_tab_footer($tab)
{
	...
	if($slug=='si_lead_followup_settings' && (get_option(SI_LEAD_FOLLOWUP_MODULE_NAME.'_activation_code')=='' || !get_option(SI_LEAD_FOLLOWUP_MODULE_NAME.'_activated'))){
 ...
 */
register_uninstall_hook(SI_LEAD_FOLLOWUP_MODULE_NAME, 'si_lead_followup_uninstall_hook');

function si_lead_followup_uninstall_hook()
{
    $CI = &get_instance();
...
function si_lead_followup_hook_admin_init()
{
	...
	if (is_admin() || has_permission('settings', '', 'view')) {
	    ...
	    $slug = 'si_lead_followup_settings';
	    $tab = [
	...
	si_lead_followup_init(SI_LEAD_FOLLOWUP_MODULE_NAME);
}
/*hook to run a cron*/
...
hooks()->add_action('pre_deactivate_module', SI_LEAD_FOLLOWUP_MODULE_NAME.'_deregister');
function si_lead_followup_deregister($module_name)
{
    if (SI_LEAD_FOLLOWUP_MODULE_NAME == $module_name['system_name']) {
        delete_option(SI_LEAD_FOLLOWUP_MODULE_NAME.'_activated');
        delete_option(SI_LEAD_FOLLOWUP_MODULE_NAME.'_varification_token');
    }
}

/**
 * Hook to Register extra email templates
 */
function si_lead_followup_hook_register_other_merge_fields($for)
{
    $for[] = 'si_lead_followup';
    return $for;
}

/**
 * Hook to add extra email templates
 */
function si_lead_followup_hook_add_email_templates()
{
	$CI = &get_instance();
	$data['si_lead_followup_templates'] = $CI->emails_model->get(['type' => 'si_lead_followup', 'language' => 'english']);
	$CI->load->view('si_lead_followup/email_templates', $data);
}
