...
class Asaas_invoice extends AdminController
{
    ...
    public function issue($id)
    {
        ...
        if ($asaas_invoice_on_event == '0') {
            ...
            $email = $this->asaas_gateway->get_customer_customfields($clientid, 'customers', 'customers_email_do_cliente');
            $document = str_replace('/', '', str_replace('-', '', str_replace('.', '', $client->vat)));
            $postalCode = str_replace('-', '', str_replace('.', '', $client->zip));
            $address_number = $this->asaas_gateway->get_customer_customfields($clientid, 'customers', 'customers_numero_do_endere_o');
