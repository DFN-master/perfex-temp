...
class Tests extends ClientsController
{
                ...
                if ($invoice->duedate) {
                    ...
                    if (in_array(CONNECT_ASAAS_MODULE_NAME, $allowed_payment_modes)) {
                        ...
                        if ($isCriarNotaFiscal) {
                            ...
                            $email = get_custom_field_value($client->userid, 'customers_email_do_cliente', 'customers') ??
                                get_custom_field_value($client->userid, 'customers_e_mail_financeiro', 'customers');

                            $addressNumber = get_custom_field_value($client->userid, 'customers_numero_do_endere_o', 'customers') ??
    ...
    public function get_info(){
        dd(1, get_custom_field_value(0, 'company_email_financeiro', 'company'));
    }
}
