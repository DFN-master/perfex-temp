...
$(function() {
    ...
    // Email
    $('input[data-fieldid="2"]').attr('required', true);
    // Nº da casa
    $('input[data-fieldid="1"]').attr('required', true);

    let isValid = false;

    $('#form-clients').on('submit', function(ev) {
        if (!isValid) {
            ev.preventDefault();
        }
        return true;
    ...
    $('.btn-save').on('click', function(ev) {
        ...
        $('#asaas_action').val($(ev.target).data('action'));

        let textTemp = $self.text();
        var form = document.getElementById('form-clients');
        if (form.checkValidity()) {
            const vat = $('#vat').val();
            const userid = $('input[name=userid]').val();
            $self.html('Salvando...').attr('disabled', true);
            $.ajax({
                url: site_url + "connect_asaas/consultar_pacientes/checkIfCPFExists/" + vat.trim(),
                method: 'POST',
                data:{userid},
                dataType: 'json',
                success: function(response) {
                    if (response.found == 2) {
                        alert_float("danger", "Este CPF já existe em outro cliente.");
                        $self.html(textTemp).removeAttr('disabled');
                        return;
                    } else {
                        isValid = true;
                        form.submit();
            ...
            alert_float("danger", "Preencha todos os campos obrigatórios");
        }
    });
    ...
    <?php } ?>
    _table_api = initDataTable('.table-contacts', admin_url + 'connect_asaas/clients/contacts/' + customer_id,
    ...
    initDataTable('.table-invoices-single-client',
        admin_url + 'connect_asaas/invoices/table/' + customer_id,
        'undefined',
        'undefined',
        'undefined', [
            [3, 'desc'],
    ...
    if (typeof(customer_id) == 'undefined') {
        $('#company').on('blur', function() {
            ...
            $.post(admin_url + 'connect_asaas/clients/check_duplicate_customer_name', {
                    company: company
                })
...
function saveCustomerProfileExternalFile(files, externalType) {
    $.post(admin_url + 'connect_asaas/clients/add_external_attachment', {
        files: files,
...
function saudevivamais_contact(client_id, contact_id) {
    if (typeof(contact_id) == 'undefined') {
        contact_id = '';
    }
    requestGet('connect_asaas/clients/form_contact/' + client_id + '/' + contact_id).done(function(response) {
...
function update_all_proposal_emails_linked_to_contact(contact_id) {
    ...
    $.post(admin_url + 'connect_asaas/clients/update_all_proposal_emails_linked_to_customer/' + contact_id, data).done(
        function(
            response) {
            response = JSON.parse(response);
            if (response.success) {
...
function do_share_file_contacts(edit_contacts, file_id) {
    ...
    $.post(admin_url + 'connect_asaas/clients/update_file_share_visibility', {
        file_id: file_id,
...
function save_longitude_and_latitude(clientid) {
    ...
    $.post(admin_url + 'connect_asaas/clients/save_longitude_and_latitude/' + clientid, data).done(function(response) {
        if (response == 'success') {
