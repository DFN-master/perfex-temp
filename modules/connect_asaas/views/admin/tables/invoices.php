...
foreach ($rResult as $aRow) {
    ...
    if (has_permission('invoices', '', 'delete') && !((min($ids) == $aRow['id'])) && !$aRow['asaas_cobranca_id']) {
        $numberOutput .= ' | <a href="' . admin_url('connect_asaas/invoices/delete/' . $aRow['id']) . '" class="text-danger _delete">' . _l('delete') . '</a>';
    }
    $numberOutput .= '</div>';
    ...
    if($aRow['asaas_cobranca_id']){
        $asaas_payment_id = preg_replace('/[^0-9]/', '', $aRow['asaas_cobranca_id']);
        $row[] = '<a href="https://www.asaas.com/i/'.$asaas_payment_id.'" target="_blank">'.$aRow['asaas_cobranca_id'].'</a>';
    }
    else{
        if(min($ids) == $aRow['id']){
            $row[] = "<span class='label label-danger'>A primeira fatura foi esta.</span>";
        }
        else{
            $row[] = $aRow['id'];
        }
