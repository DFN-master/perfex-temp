...
foreach ($rResult as $aRow) {
    ...
    $link       = admin_url('connect_asaas/subscriptions/edit/' . $aRow['id']);
    $outputName = '<a href="' . $link . '">' . $aRow['name'] . '</a>';

    $outputName .= '<div class="row-options">';

    $outputName .= '<a href="' . $aRow['asaas_subscription_link'] . '" target="_blank">' . _l('view_subscription') . '</a>';

    if (has_permission('subscriptions', '', 'edit')) {
        $outputName .= ' | <a onclick=\'editar_assinatura('.json_encode($aRow).')\' ">' . _l('edit') . '</a>';
    }
    if ((empty($aRow['stripe_subscription_id'])
        ...
        $outputName .= ' | <a href="' . admin_url('connect_asaas/subscriptions/delete/' . $aRow['id']) . '" class="text-danger _delete">' . _l('delete') . '</a>';
    }
    $outputName .= '</div>';
