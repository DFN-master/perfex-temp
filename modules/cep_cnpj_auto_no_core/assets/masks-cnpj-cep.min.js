...
function buscarValorPorChave(arr, chave) {
    arr = arr.filter(el => {
        if (el.name === chave)
            return el;
    })
...
$(document).ready(function () {
    ...
    data = JSON.parse(document.getElementById('innvea_cnpj_data').value);

    const n_razao_social = buscarValorPorChave(data, "innovea_cnpj_razao_social").value

    custom_field_whatsapp = $(`[id="custom_fields[customers][1]"]`); // WHATSAPP - CAMPO PERSONALIZADO
    custom_field_razao_social = $(`[id="custom_fields[customers][${n_razao_social}]"]`); // RAZÃO SOCIAL - CAMPO PERSONALIZADO
    custom_field_atividade_principal = $('[id="custom_fields[customers][3]"]'); // ATIVIDADE PRINCIPAL - CAMPO PERSONALIZADO
    custom_field_email = $('[id="custom_fields[customers][4]"]'); // EMAIL - CAMPO PERSONALIZADO

    cnpj.init($("#vat")), phone.init($("#phonenumber")), phone.init(custom_field_whatsapp), cep.init($("#zip"))
});

let phone = {
    init: o => {
        o.mask(phone.mascara, phone.opcoes)
    },
...
let cnpj = {
    ...
    buscar: o => {
        ...
        14 == e.length && $.ajax({
            ...
            success: function (o) {
                ...
                "ATIVA" != o.situacao && "ERROR" != o.status ? cnpj.modal_situation_not_active(o)
                        : ("OK" == o.status ? cnpj.modal_status_ok() : "ERROR" == o.status && cnpj.modal_status_error(o.message),
                                "ERROR" != o.status && cnpj.inserirDados(o))
            },
            error: function () {
    ...
    inserirDados: o => {
        ...
        $("#contact_info #zip").val(e), cep.buscar(a),
                custom_field_razao_social.val(o.nome),
                $("#contact_info #company").val(o.fantasia),
                custom_field_atividade_principal.val(o.atividade_principal[0].text + " (" + o.atividade_principal[0].code + ")"),
                custom_field_email.val(o.email),
                $("#contact_info #phonenumber").val(o.telefone),
                $("#contact_info #email").val(o.email)
    },
    modal_consultando: () => {
        Swal.fire({
    ...
    modal_try_again: () => {
        ...
        cep = {
            ...
            inserirDados: (endereco, e) => {
                ...
                $("#zip, #billing_zip, #shipping_zip").val(endereco.cep);
                $("#address, #billing_street, #shipping_street").val(logradouro + ',' + e.num + ',' + e.complemento);
