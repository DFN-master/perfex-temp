...
$("body").on('click', '.remove_template', function() {
  ...
  if ($('input[name=commission_type]:checked').val() == 'fixed') {
      $('label[for^="percent_first_invoices"]').text('<?php echo _l('commmission_first_invoices').'('._l('fixed').')'; ?>');
      $('label[for^="percent_enjoyed"]').text('<?php echo _l('commission').'('._l('fixed').')'; ?>');
      hot.updateSettings({
          colHeaders: [ '<?php echo _l('product_groups'); ?>',
                        '<?php echo _l('commission_product'); ?>',
                        '<?php echo _l('from_number'); ?>',
                        '<?php echo _l('to_number'); ?>',
                        '<?php echo _l('commission').'('._l('fixed').')'; ?>']
        });
  ...
  $('input[name=percentage_of_product_type], input[name=commission_type]').on('change', function() {
    var commission_type_text = '';
    var percentage_of_product_type_from = '';
    var percentage_of_product_type_to = '';

    if ($('input[name=commission_type]:checked').val() == 'fixed') {
      commission_type_text = '<?php echo _l('commission').'('._l('fixed').')'; ?>';
      $('label[for^="percent_first_invoices"]').text('<?php echo _l('commmission_first_invoices').'('._l('fixed').')'; ?>');
      $('label[for^="percent_enjoyed"]').text('<?php echo _l('commission').'('._l('fixed').')'; ?>');
    }
    else if ($('input[name=commission_type]:checked').val() == 'percentage') {
      commission_type_text = '<?php echo _l('commission')."(%)"; ?>';
      $('label[for^="percent_first_invoices"]').text('<?php echo _l('commmission_first_invoices')."(%)"; ?>');
      $('label[for^="percent_enjoyed"]').text('<?php echo _l('commission')."(%)"; ?>');
    }

    if ($('input[name=percentage_of_product_type]:checked').val() == 'number') {
      percentage_of_product_type_from = '<?php echo _l('from_number'); ?>';
      percentage_of_product_type_to = '<?php echo _l('to_number'); ?>';
    }
    else if ($('input[name=percentage_of_product_type]:checked').val() == 'amount') {
      percentage_of_product_type_from = '<?php echo _l('from_amount'); ?>';
      percentage_of_product_type_to = '<?php echo _l('to_amount'); ?>';
    }

    console.log($('input[name=percentage_of_product_type]').val());
    console.log(percentage_of_product_type_from);
    console.log(percentage_of_product_type_to);
    console.log($('input[name=commission_type]').val());
    console.log(commission_type_text);

    hot.updateSettings({
      colHeaders: [ 
                    '<?php echo _l('product_groups'); ?>',
                    '<?php echo _l('commission_product'); ?>',
                    percentage_of_product_type_from,
                    percentage_of_product_type_to,
