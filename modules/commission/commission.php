...
define('COMMISSION_REVISION', 109);
...
hooks()->add_filter('credits_applied', 'add_commission_credits');
hooks()->add_filter('after_payment_deleted', 'recalculate_commissions');
hooks()->add_filter('after_applied_credit_deleted', 'recalculate_commissions');
//hooks()->add_action('commission_init',COMMISSION_MODULE_NAME.'_appint');
hooks()->add_action('pre_activate_module', COMMISSION_MODULE_NAME.'_preactivate');
hooks()->add_action('pre_deactivate_module', COMMISSION_MODULE_NAME.'_predeactivate');
hooks()->add_action('pre_uninstall_module', COMMISSION_MODULE_NAME.'_uninstall');
 ...
 * @param array $data
 */
function add_commission_credits($data) {
	add_commission_credit($data);
	return true;
}
 ...
 * @param array $data
 */
function recalculate_commissions($data) {
	recalculate_commission($data);
	return true;
}

function commission_appint(){
    // Removida verificação de licença
    $CI = & get_instance();    
    // Simula ativação bem-sucedida
    set_alert('success', "The module is successfully activated.");
}

function commission_preactivate($module_name){
    if ($module_name['system_name'] == COMMISSION_MODULE_NAME) {             
        // Removida verificação de licença
        $CI = & get_instance();
        // Simula ativação sem a necessidade de verificação real
        set_alert('success', "The module is successfully activated.");
    }
}

function commission_predeactivate($module_name){
    if ($module_name['system_name'] == COMMISSION_MODULE_NAME) {
        // Removida desativação de licença
        set_alert('info', "The module has been deactivated.");
    }
}

function commission_uninstall($module_name){
    if ($module_name['system_name'] == COMMISSION_MODULE_NAME) {
        // Removida desativação de licença no desinstalar
        set_alert('info', "The module has been uninstalled.");
    }
}
