...
class Commission extends AdminController
{
    public function __construct(){
        parent::__construct();        
        hooks()->do_action('commission_init');        
    }
	/**
	 * manage commission
	 */
    ...
    public function commission_table()
    {
        if ($this->input->is_ajax_request()) {
            ...
            $select = [
                'invoice_id',
                db_prefix() . 'subscriptions.name as subscription_name',
            ...
            $join         = ['JOIN ' . db_prefix() . 'invoices ON ' . db_prefix() . 'invoices.id = ' . db_prefix() . 'commission.invoice_id',
                ...
                'LEFT JOIN ' . db_prefix() . 'subscriptions ON ' . db_prefix() . 'subscriptions.id = ' . db_prefix() . 'invoices.subscription_id',
            ];

            $result = data_tables_init($aColumns, $sIndexColumn, $sTable, $join, $where,[db_prefix() . 'invoices.clientid', 'is_client', 'amount_paid', 'subscription_id']);
            ...
            foreach ($rResult as $aRow) {
                ...
                $row[] = $_data;
                if($aRow['subscription_id'] != 0){
                    $row[] = '<a href="' . admin_url('subscriptions/edit/' . $aRow['subscription_id']) . '" target="_blank">' . $aRow['subscription_name'] . '</a>';
                }else{
                    $row[] = '<a href="' . admin_url('subscriptions/edit/' . $aRow['subscription_id']) . '" target="_blank">' . $aRow['subscription_name'] . '</a>';
                }
                ...
                if($aRow['amount_paid'] == $aRow['amount']){
                    $status_name = _l('invoice_status_paid');
                    $label_class = 'success';
                }elseif($aRow['amount_paid'] == 0){
    ...
    public function applicable_staff_table()
    {
        if ($this->input->is_ajax_request()) {
            ...
            foreach ($rResult as $aRow) {
                ...
                $row[] = _d($aRow['to_date']);
                if($aRow['is_client'] == 1){
                    $options = icon_btn('commission/delete_applicable_client/' . $aRow['applicable_staff_id'], 'fa fa-remove', 'btn-danger', ['title' => _l('delete')]);
                }else{
                    $options = icon_btn('commission/delete_applicable_staff/' . $aRow['applicable_staff_id'], 'fa fa-remove', 'btn-danger', ['title' => _l('delete')]);
                }
    ...
    public function commission_policy_table()
    {
        if ($this->input->is_ajax_request()) {
            ...
            foreach ($rResult as $aRow) {
                ...
                $options = icon_btn('commission/update_commission_policy/' . $aRow['id'], 'fa fa-edit', 'btn-default', ['title' => _l('edit')]);
                $options .= icon_btn('commission/delete_commission_policy/' . $aRow['id'], 'fa fa-remove', 'btn-danger', ['title' => _l('delete')]);
    ...
    public function receipt($id = ''){
        ...
        $data['list_agents'] = $this->commission_model->get_agents();
        ...
        foreach ($list_commission as $key => $value) {
            ...
            if(isset($receipt) && $receipt->receipt_for_a_specific_agent == 1){
                $agent = explode('_', $receipt->agent);
                if($agent[0] == 'staff' && ($agent[1] != $value['staffid'] || $value['is_client'] == 1)){
                    continue;
                }elseif($agent[0] == 'client' && ($agent[1] != $value['staffid'] || $value['is_client'] == 0)){
                    continue;
                }
            }
        ...
        if ($this->input->is_ajax_request()) {
            ...
            if ($this->input->post('salesman')) {
                ...
                foreach ($salesman as $key => $value) {
                    if($salesman_where != ''){
                        $salesman_where .= ' or id IN (SELECT receipt_id FROM ' . db_prefix() . 'commission_receipt_detail WHERE commission_id IN (SELECT id FROM ' . db_prefix() . 'commission where staffid = "'.$value.'" and is_client = 0))';
                    }else{
                        $salesman_where .= 'id IN (SELECT receipt_id FROM ' . db_prefix() . 'commission_receipt_detail WHERE commission_id IN (SELECT id FROM ' . db_prefix() . 'commission where staffid = "'.$value.'" and is_client = 0))';
                    }
                }

                if($salesman_where != ''){
                    array_push($where, 'AND ('.$salesman_where.')');
                }
            }
            ...
            if ($this->input->post('client_filter')) {
                ...
                foreach ($client_filter as $key => $value) {
                    if($client_where != ''){
                        $client_where .= ' or id IN (SELECT receipt_id FROM ' . db_prefix() . 'commission_receipt_detail WHERE commission_id IN (SELECT id FROM ' . db_prefix() . 'commission where staffid = "'.$value.'" and is_client = 1))';
                    }else{
                        $client_where .= 'id IN (SELECT receipt_id FROM ' . db_prefix() . 'commission_receipt_detail WHERE commission_id IN (SELECT id FROM ' . db_prefix() . 'commission where staffid = "'.$value.'" and is_client = 1))';
                    }
                }

                if($client_where != ''){
                    array_push($where, 'AND ('.$client_where.')');
                }
            }
                ...
                foreach ($conver_to_expense as $key => $value) {
                    if($value == 1){
                        if($conver_to_expense_where != ''){
                            $conver_to_expense_where .= ' or convert_expense = 0 or convert_expense is null';
                        }else{
                            $conver_to_expense_where .= 'convert_expense = 0 or convert_expense is null';
                        }
                    }else{
                        if($conver_to_expense_where != ''){
     ...
     * @return json
     */
    public function get_commission_by_agent(){
        if ($this->input->is_ajax_request()) {
            $this->load->model('currencies_model');
            $this->load->model('commission_model');
            $data = $this->input->post();
            ...
            if(isset($data['receipt_id']) && $data['receipt_id'] != ''){
                $where = '(amount > amount_paid or (select count(*) from '.db_prefix().'commission_receipt_detail where receipt_id = '.$data['receipt_id'].' and commission_id = '.db_prefix() . 'commission.id))';
            }else{
                $where = '(amount > amount_paid)';
            }

            if(isset($data['agent'])){
                $agent = explode('_', $data['agent']);
                if($agent[0] == 'staff'){
                    $where .= ' AND (staffid = "'.$agent[1].'" AND is_client = 0)';
                }else{
                    $where .= ' AND (staffid = "'.$agent[1].'" AND is_client = 1)';
                }
            }

            $list_commission = $this->commission_model->get_commission('', $where);
            $list_commission_return = [];
            $data['currency'] = $this->currencies_model->get_base_currency();
            $receipt_list_commission_id = [];
            $receipt_list_commission_amount = [];
            if(isset($receipt)){
