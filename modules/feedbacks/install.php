...
add_option('feedback_send_emails_per_cron_run', 100);
add_option('last_feedback_send_cron', '');

if (!$CI->db->table_exists(db_prefix() . 'feedbackresultsets')) {
    $CI->db->query('CREATE TABLE `' . db_prefix() . 'feedbackresultsets` (
  ...
  `date` datetime NOT NULL
    ...
    $CI->db->query('ALTER TABLE `' . db_prefix() . 'feedbackresultsets`
...
if (!$CI->db->table_exists(db_prefix() . 'feedbacksemailsendcron')) {
    $CI->db->query('CREATE TABLE `' . db_prefix() . 'feedbacksemailsendcron` (
  `id` int(11) NOT NULL,
  `feedbackid` int(11) NOT NULL,
  ...
  `log_id` int(11) NOT NULL
    ...
    $CI->db->query('ALTER TABLE `' . db_prefix() . 'feedbacksemailsendcron`
  ADD PRIMARY KEY (`id`);');
...
if (!$CI->db->table_exists(db_prefix() . 'feedbacks')) {
    $CI->db->query('CREATE TABLE `' . db_prefix() . "feedbacks` (
  `feedbackid` int(11) NOT NULL,
  ...
  `hash` varchar(32) NOT NULL
    ...
    $CI->db->query('ALTER TABLE `' . db_prefix() . 'feedbacks`
...
if (!$CI->db->table_exists(db_prefix() . 'feedbacksendlog')) {
    $CI->db->query('CREATE TABLE `' . db_prefix() . "feedbacksendlog` (
  `id` int(11) NOT NULL,
  `feedbackid` int(11) NOT NULL,
  ...
  `send_to_mail_lists` text
    ...
    $CI->db->query('ALTER TABLE `' . db_prefix() . 'feedbacksendlog`
  ADD PRIMARY KEY (`id`);');
