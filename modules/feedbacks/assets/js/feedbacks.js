$(function() {
    ...
    $('input[name="send_feedback_to[leads]"]').on('change', function() {
        $('.leads-statuses').slideToggle();
    });

    $('input[name="send_feedback_to[clients]"]').on('change', function() {
        $('.customer-groups').slideToggle();
    });

    $('.feedback-customer-groups input').on('change', function() {
        if ($('.feedback-customer-groups input:checked').length > 0) {
            $('#ml_customers_all').prop('checked', false);
        }
    });

    $('.feedback-lead-status input').on('change', function() {
        if ($('.feedback-lead-status input:checked').length > 0) {
    ...
    $('#ml_customers_all').on('change', function() {
        if ($(this).prop('checked') !== false) {
            $('.feedback-customer-groups input').prop('checked', false);
        }
    });

    $('#ml_leads_all').on('change', function() {
        if ($(this).prop('checked') !== false) {
            $('.feedback-lead-status input').prop('checked', false);
        }
    });
    ...
    var questions_sortable = $("#feedback_questions").sortable({
        placeholder: "ui-state-highlight-feedback",
        update: function() {
    ...
    // Add merge field
    $('.add_email_list_custom_field_to_feedback').on('click', function(e) {
        e.preventDefault();
...
function feedback_toggle_full_view() {
    $('#feedback-add-edit-wrapper').toggleClass('hide');
    $('#feedback_questions_wrapper').toggleClass('col-md-12');
    $('#feedback_questions_wrapper').toggleClass('col-md-7');
}
// New feedback question
function add_feedback_question(type, feedbackid) {
    $.post(admin_url + 'feedbacks/add_feedback_question', {
        type: type,
        ...
        question_area += '<label>' + response.feedback_question_required + '</label>';
        question_area += '</div>';
        question_area += hidden_input('order[]', '');
        // used only to identify input key no saved in database
        question_area += '<label for="' + response.data.questionid + '" class="control-label display-block">' + response.feedback_question_string + ' <a href="#" onclick="update_question(this,\'' + type + '\',' + response.data.questionid + '); return false;" class="pull-right update-question-button"><i class="fa fa-refresh text-success question_update"></i></a><a href="#" class="pull-right"><i class="fa fa-remove text-danger" onclick="remove_question_from_database(this,' + response.data.questionid + '); return false;"></i></a></label>';
        ...
        if (type == 'textarea') {
            question_area += '<textarea class="form-control mtop20" disabled="disabled" rows="6">' + response.feedback_question_only_for_preview + '</textarea>';
            ...
            question_area += '<label><input onblur="update_question(this,\'' + type + '\',' + response.data.questionid + ');" type="text" data-box-descriptionid="' + response.data[0].questionboxdescriptionid + '" class="feedback_input_box_description"></label>';
            question_area += '</div>';
            question_area += '</div>';
            question_area += '</div>';
            // end box row
            ...
            question_area += '<input type="text" onchange="update_question(this,\'' + type + '\',' + response.data.questionid + ');" class="form-control mtop20" disabled="disabled" value="' + response.feedback_question_only_for_preview + '">';
        }
        question_area += '</div>';
        question_area += '</li>';
        $('#feedback_questions').append(question_area);
        $("#feedback_questions").sortable('refresh');
        $('html,body').animate({
                scrollTop: $("#feedback_questions li:last-child").offset().top
            },
            'slow');
        update_questions_order();
    });
...
function update_question(question, type, questionid) {
    ...
    if (type == 'checkbox' || type == 'radio') {
        ...
        $.each(boxes_area, function() {
            var boxdescriptionid = $(this).find('input.feedback_input_box_description').data('box-descriptionid');
            var boxdescription = $(this).find('input.feedback_input_box_description').val();
    ...
    setTimeout(function() {
        $.post(admin_url + 'feedbacks/update_question', data).done(function(response) {
...
function add_more_boxes(question, boxdescriptionid) {
    ...
    $(box).find('input.feedback_input_box_description').val('');
    $(box).find('input.feedback_input_box_description').attr('data-box-descriptionid', boxdescriptionid);
    $(box).find('input.feedback_input_box_description').focus();
...
function remove_question_from_database(question, questionid) {
    $.get(admin_url + 'feedbacks/remove_question/' + questionid, function(response) {
...
function remove_box_description_from_database(question, questionboxdescriptionid) {
    $.get(admin_url + 'feedbacks/remove_box_description/' + questionboxdescriptionid, function(response) {
...
function add_box_description_to_database(question, questionid, boxid) {
    $.get(admin_url + 'feedbacks/add_box_description/' + questionid + '/' + boxid, function(response) {
...
// Updating feedback question order // called when drop event called
function update_questions_order() {
    var questions = $('#feedback_questions').find('.question');
    var i = 1;
    ...
    data = {};
    data.data = update;
    $.post(admin_url + 'feedbacks/update_feedback_questions_orders', data);
}
