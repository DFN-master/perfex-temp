...
$where[] = 'AND '.db_prefix().'items.can_be_product_service = "can_be_product_service" AND  '.db_prefix().'items.service_type = "subscriptions"';


$result = data_tables_init($aColumns, $sIndexColumn, $sTable, $join, $where, [db_prefix().'items.id', 'commodity_code', 'subscription_price', 'subscription_count', 'subscription_period']);
	...
	for ($i = 0; $i < count($aColumns); $i++) {
		...
		if ($aColumns[$i] == 'description') {
			...
			if (has_permission('service_management', '', 'edit') || is_admin()) {
				$code .= ' | <a href="' . admin_url('service_management/add_edit_subscription/' . $aRow['id']) . '"  >' . _l('edit') . '</a>';
			}
			if (has_permission('service_management', '', 'delete') || is_admin()) {
				$code .= ' | <a href="' . admin_url('service_management/delete_product/' . $aRow['id'].'/subscription') . '" class="text-danger _delete">' . _l('delete') . '</a>';
			}
			...
			$subtext = app_format_money($aRow['subscription_price'], $base_currency_id);
			if ($aRow['subscription_count']  == 1) {
				$subtext .= ' / ' .$aRow['subscription_period'];
			} else {
				$subtext .= ' (every ' .$aRow['subscription_count'] . ' ' .$aRow['subscription_period'] . 's)';
			}
			$_data =   '<span class="label label-success  tag-id-1"><span class="tag">' .$subtext. '</span></span>';
		} else if($aColumns[$i] == '3' ) {
			$_data = '<a href="'.site_url('service_management/service_management_client/detail/'. $aRow['id']).'">'.site_url('service_management/service_management_client/detail/'. $aRow['id']).'</a>';
		} else if($aColumns[$i] == 'long_description') {
			$_data = $aRow['long_description'];
		}
