...
<div class="btn-group pull-right btn-with-tooltip-group _filter_data" data-toggle="tooltip" data-title="<?php echo _l('filter_by'); ?>">
    ...
    <ul class="dropdown-menu dropdown-menu-left width300 height500">
        ...
        <?php if(count($years) > 0){ ?>
            ...
            <?php foreach($years as $year){ ?>
                <li class="active">
                    <a href="#" data-cview="year_<?php echo new_html_entity_decode($year['year']); ?>" onclick="dt_custom_view(<?php echo new_html_entity_decode($year['year']); ?>,'.table-contracts','year_<?php echo new_html_entity_decode($year['year']); ?>'); return false;"><?php echo new_html_entity_decode($year['year']); ?>
                </a>
            </li>
        <?php } ?>
    <?php } ?>
    ...
    <li class="dropdown-submenu pull-left">
        ...
        <ul class="dropdown-menu dropdown-menu-left">
            <?php for ($m = 1; $m <= 12; $m++) { ?>
                <li><a href="#" data-cview="contracts_by_month_<?php echo new_html_entity_decode($m); ?>" onclick="dt_custom_view(<?php echo new_html_entity_decode($m); ?>,'.table-contracts','contracts_by_month_<?php echo new_html_entity_decode($m); ?>'); return false;"><?php echo _l(date('F', mktime(0, 0, 0, $m, 1))); ?></a></li>
            <?php } ?>
        </ul>
    </li>
    <div class="clearfix"></div>
    <?php if(count($contract_types) > 0){ ?>
        ...
        <?php foreach($contract_types as $type){ ?>
            <li>
                <a href="#" data-cview="contracts_by_type_<?php echo new_html_entity_decode($type['id']); ?>" onclick="dt_custom_view('contracts_by_type_<?php echo new_html_entity_decode($type['id']); ?>','.table-contracts','contracts_by_type_<?php echo new_html_entity_decode($type['id']); ?>'); return false;">
                    <?php echo new_html_entity_decode($type['name']); ?>
                </a>
            </li>
        <?php } ?>
    <?php } ?>
