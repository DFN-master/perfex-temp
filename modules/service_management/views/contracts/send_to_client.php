...
<div class="modal fade email-template" data-editor-id=".<?php echo 'tinymce-'.$contract->id; ?>" id="contract_send_to_client_modal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
    <div class="modal-dialog" role="document">
        <?php echo form_open('service_management/contracts/send_to_email/'.$contract->id); ?>
                    ...
                    <div class="row">
                        <div class="col-md-12">
                            ...
                            <?php foreach($contract->attachments as $attachment) { ?>
                                <div class="checkbox checkbox-primary">
                                    <input type="checkbox" <?php if(!empty($attachment['external'])){echo 'disabled';}; ?> value="<?php echo new_html_entity_decode($attachment['id']); ?>" name="email_attachments[]">
                                    <label for=""><a href="<?php echo site_url('download/file/contract/'.$attachment['attachment_key']); ?>"><?php echo new_html_entity_decode($attachment['file_name']); ?></a></label>
                                </div>
                                <?php } ?>
                            </div>
                        </div>
