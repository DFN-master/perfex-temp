<script>
	...
	function wh_delete_item(row, itemid,parent) {
		...
		var id_product = itemid;  
		var service_id_list = getCookie('service_id_list'), service_qty_list;

		if(typeof service_id_list != ""){
			if(service_id_list.trim()){
				var id_list = JSON.parse('['+service_id_list+']');
				service_qty_list = getCookie('service_qty_list');
				var qty_list = JSON.parse('['+service_qty_list+']');
				var empty = 0;
				var new_list_id = []; 
				var index = -1;
...
function sm_add_item_to_table(data, itemid) {
	...
	function init_goods_delivery_currency(id, callback) {
		...
		if ($accountingTemplate.length || id) {
			...
			requestGetJSON('service_management/service_management_client/get_currency/' + selectedCurrencyId)
