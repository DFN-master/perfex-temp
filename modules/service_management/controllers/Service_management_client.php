...
class Service_management_client extends ClientsController
{
	...
	public function products_service_managements($page= 1,$id = 0, $warehouse = 0,$key = '')
	{
			...
			$data['title_group'] = _l('sm_all_products');
			$data['page'] = $page;
	...
	public function view_cart($id = ''){
		...
		if(isset($_COOKIE['service_id_list']) && new_strlen($_COOKIE['service_id_list']) > 0){
			...
			$arr_item_billing_id = new_explode(',', $item_billing_id);
			foreach ($arr_item_billing_id as $key => $billing_id) {
				$this->db->where('id IN('.$billing_id.')');
				$sm_items_cycles = $this->db->get(db_prefix().'sm_items_cycles')->result_array();

				if(isset($sm_items_cycles[0])){
				$billing_value = $sm_items_cycles[0];
